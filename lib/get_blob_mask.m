function mask = get_blob_mask(img)
  ...
  if exist('bwfill')
    ...
    mask = false(sz(1:2));
    mask(interface) = true;
    mask = bwfill(mask, 'holes'); % image toolbox
  else
    mask = via_propagation(size(img), interface); % slower
  end
end
  
  
function mask = via_propagation(sz, interface)
